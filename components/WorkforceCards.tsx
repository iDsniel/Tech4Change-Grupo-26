...
export default function WorkforceCards({ data, workspace }: Props) {
  ...
  return <>
    <section className="detailCard">
      ...
      <p>O Workforce KPI adiciona uso agregado por cartão no período da própria fonte. Nomes não entram no contrato. Eventos continuam sendo evidências de associação, não prova de responsabilidade do operador.</p>
      ...
      {workforce ? <p><strong>Workforce KPI:</strong> {workforce.periodStart} a {workforce.periodEnd} · granularidade cartão-período · unidade métrica. Os totais trimestrais não são repartidos por mês ou dia.</p> : <p>Esta base ainda não contém Workforce KPI atual. Importe um JSON v3 preparado com o relatório Workforce.</p>}
      {month !== "all" && workforceCard && <p><strong>Atenção à granularidade:</strong> o filtro {month} afeta somente os eventos abaixo. Os contadores Workforce permanecem referentes a {workforce?.periodStart} a {workforce?.periodEnd}.</p>}
      <section className="kpiGrid">
        <article className="kpiCard"><div><span>Cartões Workforce</span><strong>{workforce?.cards.length ?? 0}</strong><small>linhas pai do relatório, sem nomes</small></div></article>
        ...
        <article className="kpiCard"><div><span>Códigos sem KPI atual</span><strong>{eventOnlyCodes.length}</strong><small>presentes em eventos, sem correspondência exata no Workforce</small></div></article>
        <article className="kpiCard"><div><span>Códigos a revisar</span><strong>{qualityRows.length}</strong><small>incompletos ou ambíguos no Workforce</small></div></article>
      </section>
      {(eventOnlyCodes.length > 0 || workforceOnlyCodes.length > 0 || qualityRows.length > 0) && <details>
        <summary>Conciliação de códigos</summary>
        {eventOnlyCodes.length > 0 && <p>Somente em eventos: {eventOnlyCodes.join(", ")}. Sem KPI Workforce correspondente, o Pulso não infere equivalência.</p>}
        {workforceOnlyCodes.length > 0 && <p>Somente no Workforce: {workforceOnlyCodes.join(", ")}. Ausência de evento não significa ausência de uso.</p>}
        {qualityRows.length > 0 && <p>Linhas Workforce a revisar: {qualityRows.map(row => `linha ${row.sourceRow} (${row.cardQuality})`).join(", ")}.</p>}
        <p>Códigos são tratados como texto e nunca recebem preenchimento automático. Se outra fonte exportar cartão como número, zeros à esquerda não podem ser reconstruídos com segurança.</p>
      </details>}
    </section>
    ...
    {card !== "all" && <section className="detailGrid">
      <article className="detailCard">
        ...
        {workforceMatches.length > 1 && <p>Código duplicado no Workforce. As linhas estão marcadas como ambíguas e não são agregadas automaticamente.</p>}
        {workforceMatches.length === 0 && <p>Não há recorte Workforce para este código. A visão abaixo permanece limitada aos eventos existentes.</p>}
        {workforceCard && asset !== "all" && !workforceAsset && <p>O cartão não possui linha Workforce para {asset}; nenhum total foi inferido a partir do equipamento.</p>}
        {workforceCard && (asset === "all" || workforceAsset) && <>
          ...
          <p>Os indicadores são contadores agregados do período do Workforce. Não são diagnóstico, produtividade individual, ranking ou atribuição de causa.</p>
        </>}
      </article>
      <article className="detailCard">
    ...
    {workforceCard && asset === "all" && <section className="detailCard">
      <h2>Equipamentos associados ao cartão no Workforce</h2>
      <p>Linhas filhas da origem são recortes do total do cartão e não registros adicionais. Não some esta tabela ao total do cartão.</p>
    ...
    {card !== "all" && <section className="detailCard">
      ...
      <p>O Workforce fornece um único total para todo o período. Por isso, a evolução mensal abaixo usa somente os eventos com o código selecionado; horas e distância trimestrais não são distribuídas artificialmente.</p>
    ...
    {card !== "all" && workforceCard && <section className="detailCard">
      ...
      <p>As ordens abaixo pertencem aos equipamentos que aparecem no recorte Workforce do cartão; isso cria contexto operacional, não responsabilidade individual. O antes/depois continua calculado apenas com a série diária do equipamento.</p>
