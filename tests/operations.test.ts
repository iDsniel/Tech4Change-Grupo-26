...
const input = (): DailyInput => ({ id: "input-1", assetId: "EP01", date: "2026-06-01", plannedHours: 8, downtimeHours: 2, fuelQuantity: 10, fuelUnit: "L", costBRL: 100, production: 30, productionUnit: "t", usageContext: "production", note: "Fonte manual" });
...
test("availability uses only measured days; fuel and production units remain separate", () => {
  ...
  assert.equal(s.covered, 1); assert.equal(s.availability, 75); assert.equal(s.fuelL, 10); assert.equal(s.fuelKg, 5); assert.equal(s.tonnes, 30); assert.equal(s.movements, 30); assert.equal(s.pallets, 0);
...
test("pallets and maintenance context are accepted without inventing production", () => {
  const row = validateInput({ ...input(), production: 42, productionUnit: "pallets", usageContext: "maintenance" });
  assert.equal(row.productionUnit, "pallets");
  assert.equal(row.usageContext, "maintenance");
  const s = inputSummary([row]);
  assert.equal(s.pallets, 42);
  assert.equal(s.tonnes, 0);
});
